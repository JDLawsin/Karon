-- Keep length bounds in sync with clinicNameSchema in apps/clinic.

update public.clinics
set name = case
  when char_length(btrim(name)) < 2 then 'Clinic'
  else left(btrim(name), 80)
end
where char_length(btrim(name)) < 2
   or char_length(btrim(name)) > 80
   or name <> btrim(name);

alter table public.clinics
  add constraint clinics_name_length
